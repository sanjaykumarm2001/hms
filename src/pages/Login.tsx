...
import { ArrowRightIcon, AtSignIcon, EyeIcon, EyeOffIcon, KeyRoundIcon, ShieldCheckIcon } from 'lucide-react';
    ...
    <div className="relative flex h-full w-full flex-col items-center justify-center overflow-y-auto bg-brand-gradient px-6 py-10">
      <div className="w-full max-w-[380px]">
        <div className="flex flex-col items-center">
          <span className="flex h-12 w-12 items-center justify-center rounded-[12px] bg-white shadow-panel">
            <svg viewBox="0 0 20 20" className="h-7 w-7" aria-hidden="true">
              <path d="M10 2.5 17 6v5.2c0 3.2-2.8 5.6-7 6.3-4.2-.7-7-3.1-7-6.3V6l7-3.5Z" fill="#25b84f" />
        ...
        <div className="mt-7 rounded-[12px] bg-white p-7 shadow-panel">
          ...
          <p className="mt-1 text-center text-[12px] text-ink-muted">Sign in to your ProPMS Sentinel account</p>

          <form className="mt-6 space-y-4" onSubmit={handleSubmit} noValidate>
            <div>
              <label htmlFor="username" className="text-[10px] font-semibold uppercase tracking-[0.12em] text-ink-soft">
                Username
              </label>
              <div className="relative mt-2">
                <AtSignIcon aria-hidden="true" className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-ink-muted" />
                <input
                  id="username"
                  type="email"
            ...
            <div>
              <div className="flex items-center justify-between">
                <label htmlFor="password" className="text-[10px] font-semibold uppercase tracking-[0.12em] text-ink-soft">
                  Password
                </label>
                <button type="button" className="text-[11px] font-semibold text-brand-700 hover:text-brand-800">
                  Forgot password?
                </button>
              </div>
              <div className="relative mt-2">
                <KeyRoundIcon aria-hidden="true" className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-ink-muted" />
                <input
                  id="password"
            ...
            {error ?
            <p role="alert" className="rounded-md bg-[#fdeceb] px-3 py-2 text-[12px] font-medium text-[#b3312a]">
                {error}
              </p> :
            null}
        ...
        <p className="mt-7 text-center text-[10.5px] text-white/80">
          © 2024 Meridian Hotel Management Group. All rights reserved. · Protected by Sentinel Access Layer v2.4
        </p>
      </div>
    </div>);
